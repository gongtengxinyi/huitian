...
function($scope, $http, $window, UrlUtil) {
  ...
  $scope.query = function() {
    $http.get('userContact/list.do?indentId=' + $scope.data.indentId).then(function(response) {
  ...
  $scope.goBack = function() {
    $window.location.href = UrlUtil.transform('indent/indentList.do?id='+$scope.data.indentId+'&&pageNo='+pageNo);
  }
