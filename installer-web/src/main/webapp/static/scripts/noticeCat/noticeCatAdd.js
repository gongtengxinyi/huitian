...
app.controller('noticeCatAddController',//
  ...
  $scope.save = function() {
    ...
    return $http({
        ...
        if (data.success) {
          ...
          $window.location.href = UrlUtil.transform('noticeCat/noticeCatList.do');
        } else {
      	  $("#saveBtn").attr("disabled", false);
          var modalScope = $scope.$new(true);
          modalScope.title = data.err_msg;
          modalScope.message = "添加公告失败";
          $uibModal.open({
            templateUrl : 'template/modal/alert.html',
            scope : modalScope
          }); 
