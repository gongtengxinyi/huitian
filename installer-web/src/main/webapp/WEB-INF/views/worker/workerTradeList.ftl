...
<title>工匠提现管理查询</title>
<#include "/include/common-list.ftl" />
<script src="static/scripts/account/workerTradeList.js?randomId=<%=Math.random()%>"></script>
</head>
<body>
  <#include "/include/body-begin.ftl" />
  <div ng-controller="WorkerTradeListController">

    <div class="panel panel-default form-horizontal">
      <div class="panel-heading">工匠提现管理查询</div>
        ...
        <tbody ng-repeat="data in rows">
          <tr>
            ...
            <td><span ng-if="data.tradeStatus == 'TREATMENT'">
                <a href="workerTrade/workerTradeEdit.do?id={{data.id}}&&status=0"
                class="btn btn-success btn-xs">查看详情</a>
            </span>
            <span ng-if="data.tradeStatus != 'TREATMENT'">
                <a href="workerTrade/workerTradeEdit.do?id={{data.id}}&&status=1"
