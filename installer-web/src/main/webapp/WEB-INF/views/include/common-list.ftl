
<script src="static/angular/angular.gzjs"></script>
<script src="static/angular/i18n/angular-locale_zh-cn.js"></script>
<script src="static/angular-ui/ui-bootstrap-tpls.gzjs"></script>
<script src="static/scripts/core/ifu-template.js"></script>
<script src="static/scripts/core/ifu-form.js"></script>
<script src="static/scripts/core/ifu-util.js"></script>
<script src="static/js/numeric-directive.gzjs"></script>

<script src="static/scripts/core/common.js"></script>