	...
	//添加所有品类项及其数据
	                ...
	                angular.forEach($scope.rt.itemPriceList1, function(data) {
	                  if (data.serveContentId == content.id) {
	                      $("#offer tbody").append('<tr><td id="typeName'+index+'">'+type.name1
	                       ...
	                       +'</td><td>￥<input type="text" id="actualPrice'+index+'" value="'+0
	                       +'"> </td></tr>');
	 ...
	 $scope.save = function() {
		 ...
		 if (!$scope.form1.$valid) {
		    ...
		    console.log($scope.data.apartyPriceList);
        

		 $http.post('aparty/doSavePrice.do', $scope.data)
		    .then(function(response) {
		      var data = response.data
		    
		      if (data.success) {
		        var modalScope = $scope.$new(true);
		        modalScope.message = "成功录入报价信息，返回甲方列表";
