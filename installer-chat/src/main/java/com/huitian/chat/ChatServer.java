...
import com.huitian.api.account.service.impl.CenterAccountServiceImpl;
...
public class ChatServer {
	 ...
	 * 是否是第一次推送的标记
	 */
	private static ConcurrentHashMap<String, String> centerAccountIdToIndentStatus = new ConcurrentHashMap<String, String>();
	...
	public void onOpen(Session session) {
		...
		if (!parseQueryString(session)) // 如果未能取得用户id和type，退出
			return;
		try {
			CenterAccount centerAccount = checkCenterAccount();// 检查是否是系统的账号
	 ...
	 * 检查是否是系统账号
	 */
	private CenterAccount checkCenterAccount() {
		// TODO Auto-generated method stub
		CenterAccountService centerAccountService = getCenterAccountService();
		return centerAccountService.find(this.getCenterAccountId());
	}

	public CenterAccountService getCenterAccountService() {
		return SpringContextHolder.getBean(CenterAccountServiceImpl.class);
	}
	...
	private boolean parseQueryString(Session session) {
		...
		String centerAccountStart = "CENTERACCOUNTID="; // 查询字符串的每一节，以&隔开，传递用户id的部分以userID开头，必须有
		...
		try {
			...
			for (int i = 0; i < queryStringArray.length; i++) {// ‘对查询字符串进行检查，如果有uid等参数，则取出病赋值
				if (queryStringArray[i].toUpperCase().startsWith(terminalStart)) {// 用户id
					this.setTerminalType(queryStringArray[i].split("=")[1]);
				}
				if (queryStringArray[i].toUpperCase().startsWith(centerAccountStart)) {// 类型
					this.setCenterAccountId(queryStringArray[i].split("=")[1]);
				}
			}
	...
	private void parseChatMessage(ChatMessage chatMessage) {
		if (StringUtils.equals(chatMessage.getMessageMode(), EnumMessageMode.APP_PUSH_INDENT.name())) {
			...
			try {
				...
				if (StringUtils.isBlank(status) || StringUtils.equals("NO", status)) {
			...
			String centerAccountId = chatMessage.getMessage();
			ChatServer chatServer = centerAccountIdToChatServer.get(centerAccountId);
			String lowDpiKey = JacksonHelper.toJson(chatMessage);
			chatServer.sendMessage(lowDpiKey);
	...
	private String createChatMessage(IndentDto indent) {
		...
		if (indent == null) {
			chatMessage.setMessageMode(EnumMessageMode.NO_INDENT.name());
		} else {
			chatMessage.setMessageMode(EnumMessageMode.HAVE_INDENT.name());
			String message = gson.toJson(indent);
			chatMessage.setMessage(message);
		}
		chatMessage.setMessageType(EnumMessageType.TEXT.name());
